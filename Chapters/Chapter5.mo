within Chapters;
package Chapter5 "Naming Conventions, Units, and Package Design"

  model UnitTest "Deliberately triggers a unit violation"
    Modelica.Units.SI.Temperature T "Fluid temperature (K)";
    Modelica.Units.SI.Mass m "Total fluid mass (kg)";
  equation
    T = m + 300;
    annotation(
      Icon(
      ...
      Documentation(info = "<html><p>This model deliberately mixes Temperature (K) and Mass (kg) in one equation to demonstrate unit checking. Validate to see the unit violation warning/error.</p></html>")
    );
  end UnitTest;

  package TryThis "Try This exercises for Chapter 5"

    model UnitTestFixed "Fixed version: dimensionally consistent"
      Modelica.Units.SI.Temperature T "Fluid temperature (K)";
    equation
      T = 300;
      annotation(
        ...
        experiment(StopTime = 1),
        Documentation(info = "<html><p>Try This: The fixed version of UnitTest. The mass variable is removed and T is set to a dimensionless constant, making the equation physically meaningful.</p></html>")
      );
    end UnitTestFixed;
  ...
  annotation(
    ...
    Documentation(info = "<html><p>Chapter 5: Naming Conventions, Units, and Package Design. Covers UpperCamelCase vs lowerCamelCase, SI unit types, unit checking, documentation, and package organization.</p></html>")
  );
end Chapter5;
